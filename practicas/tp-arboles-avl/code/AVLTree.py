...
def calculateBalance(tree:AVLTree):
    calcularAltura(tree.root)
    return

def calcularAltura(avlnode:AVLNode):
    if avlnode.leftnode==None and avlnode.rightnode==None:
        avlnode.bf=0
        return 1
    elif avlnode.leftnode==None:
        hd=calcularAltura(avlnode.rightnode)
        avlnode.bf=0-hd
        return 1+hd
    elif avlnode.rightnode==None:
        hi=calcularAltura(avlnode.leftnode)
        avlnode.bf=hi-0
        return 1+hi
    else:
        hi=calcularAltura(avlnode.leftnode)
...
def recorreInOrder(avlnode:AVLNode,l:list):
  if avlnode.leftnode!=None:                                   #para hacer el recorrido InOrder primero trato de llegar
    recorreInOrder(avlnode.leftnode,l)                         #al ultimo nodo a la izq, luego se agrega el nodo en el que se esta
  l.append(avlnode.value)                                         #y trata de ir hacia la derecha
  if avlnode.rightnode!=None:
    recorreInOrder(avlnode.rightnode,l)
  return
...
def traverseInOrder(tree:AVLTree):                             
  ...
  if tree.root!=None:                                       #si el arbol no esta vacio llamamos a la funcion recursiva
    recorreInOrder(tree.root,listaInOrder)                  #doy vuelta el sentido de los nodos, ya que el que se agrega primero
                                                            #es el ultimo en la lista
    return listaInOrder
  else:
    return None

def recorrePostOrder(avlnode:AVLNode,l:list):
  if avlnode.leftnode!=None:                                   #Para recorrer en PostOrder, primero tratamos de llegar a las hojas de un nodo
    recorrePostOrder(avlnode.leftnode,l)                       #Se agregan las hojas, y luego el nodo padre.
  if avlnode.rightnode!=None:
    recorrePostOrder(avlnode.rightnode,l)
  l.append(avlnode.value)
  return

def traverseInPostOrder(tree:AVLTree):
  listaPostOrder=[]                                          #Creamos la lista a regresar con los nodos ordenados
  if tree.root!=None:                                        #si el arbol no esta vacio llamamos a la funcion recursiva
    recorrePostOrder(tree.root,listaPostOrder)               #doy vuelta el sentido de los nodos, ya que el que se agrega primero
                                                             #es el ultimo en la lista
    return listaPostOrder
  else:
    return None

def recorrePreOrder(avlnode:AVLNode,l:list):
  l.append(avlnode.value)                                         #Para recorrer en preorder, primero agregamos el nodo en el que estamos
  if avlnode.leftnode!=None:                                   #y luego pasamos a sus hojas
    recorrePreOrder(avlnode.leftnode,l)
  if avlnode.rightnode!=None:
    recorrePreOrder(avlnode.rightnode,l)
  return

def traverseInPreOrder(tree:AVLTree):
  listaPreOrder=[]                                           #Creamos la lista a regresar con los nodos ordenados
  if tree.root!=None:                                        #si el arbol no esta vacio llamamos a la funcion recursiva
    recorrePreOrder(tree.root,listaPreOrder)                 #doy vuelta el sentido de los nodos, ya que el que se agrega primero
                                                             #es el ultimo en la lista
    return listaPreOrder
  else:
    return None

def recorreSearch(avlnode:AVLNode, elem):
  if avlnode.value==elem:                                      #evaluamos el valor del nodo en el que estamos, con el elemento a
    ...
    if avlnode.leftnode!=None:                                 #se quedara con none.
      resp=recorreSearch(avlnode.leftnode,elem)                #si el nodo tiene nodos hijo, se llama a la funcion recursiva 
    ...
    if avlnode.rightnode!=None:                                #Y tanto si lo encuentra como si no, va a regresar el valor de resp
      resp=recorreSearch(avlnode.rightnode,elem)
    return resp
      
def search(tree:AVLTree,elem):
  if tree.root!=None:                                        #si el arbol no esta vacio llamamos a la funcion recursiva
    return recorreSearch(tree.root,elem)
  else:
    return None

def recorreInsert(avlnode:AVLNode,nod:AVLNode):
  if nod.key==avlnode.key:                                     #Al insertar un nodo primero verificamos que no haya key's duplicadas
    ...
    if avlnode.leftnode==None:                                 #y el nodo actual no tiene elemento en su rama izquierda
      ...
      return recorreInsert(avlnode.leftnode,nod)               #con el nodo de la izq
  else:
    if avlnode.rightnode==None:                                #si la key, no es ni igual ni menor, solo queda que es mayor, y se
      ...
      return recorreInsert(avlnode.rightnode,nod)
...
def insert(tree:AVLTree,elem, key):
  ...
  if tree.root==None:                                        #si el arbol esta vacio, el elemento a insertar queda como raiz del arbol
    ...
    return recorreInsert(tree.root,nodo)

def recorreDelete(avlnode:AVLNode, elem):
  if avlnode.value==elem:                                      #Una vez que encuentro el nodo que tiene al elemento que busco eliminar
    if avlnode.leftnode==None and avlnode.rightnode==None:          #debo verificar 4 posibles casos, que el nodo no tenga nodos hijos, tenga
      ...
      if nodoTemp!=None:                                    #si existe una rama derecha del nodo que remplazara
        ...
        recorreInsert(nodoAct,nodoTemp)
    return avlnode.key
  else:                                                     #si no es el nodo que se busca, se ve si tiene nodos hijos para seguir la busqueda
    resp=None
    if avlnode.leftnode!=None:
      resp=recorreDelete(avlnode.leftnode,elem)
    if resp!=None:
      return resp
    if avlnode.rightnode!=None:
      resp=recorreDelete(avlnode.rightnode,elem)
    return resp

def delete(b:AVLTree,elem):
  if b.root!=None:                                          #Si el arbol no esta vacio, se llama a la funcion recursiva con el nodo raiz
    return recorreDelete(b.root,elem)                       #como primer nodo
  else:
    return None
  ...
  if avlnode.key==key:                                           #Una vez que encuentro el nodo que tiene la key que busco eliminar
    if avlnode.leftnode==None and avlnode.rightnode==None:            #debo verificar 4 posibles casos, que el nodo no tenga nodos hijos, tenga
      ...
      if avlnode.parent.leftnode.key==key:                       #se ve de que parte es hijo el nodo a eliminar para actualizar referencias
        ...
        avlnode.parent.rightnode=nodoAct
      if nodoTemp!=None:                                    #si existe una rama derecha del nodo que remplazara, 
        recorreInsert(nodoAct,nodoTemp)                     #se lo inserta en el arbol nuevamente
    return avlnode.key
  else:                                                     #si no es el nodo que se busca, se ve si tiene nodos hijos para seguir la busqueda
    resp=None
    if avlnode.key>key:
